...
<body>
    ...
    <sql:query dataSource="${xesrc}" var="empdept"> 
    select rowidtochar(a.rowid) rowid1,a.EMPID,b.EMPNAME,a.DEPTID,c.DEPTNAME from mapemployee a,employee b,department c 
	where a.empid = b.empid and a.deptid =c.deptid  order by a.empid
    </sql:query>
	...
	<form action="SubmitGridMap" method="post" name="form2">
		<table id="table2" class="table table-bordered table-striped" border="1" cellpadding="0" cellspacing="0"
			...
			<%int i = 0; %>
            <tbody>
			<c:forEach var="m" items="${empdept.rows}">
			<%
				i++;
			%>
			<tr>
				...
				<td valign="top"><input type="hidden"
					name="erid<%out.print(i);%>" value="${m.rowid1}" />
